...
describe('[ *** US#8.1 Update answer *** ]', () => {
    ...
    describe('@@@ SCENARIO 1 – All data entered correctly @@@', () => {
        ...
        it('gets back updated answer object', (done) => {
                ...
                request(app).post('/api/auth/signup').send(testData.user1).set('Accept', 'application/json').expect('Contect-Type', /json/).expect(200).end((err, res) => {
                    ...
                    Answer.sync({ force: true }).then(() => {
                        request(app).post('/api/answers').send(testData.answer2).set('Authorization', `Bearer ${token}`).set('Accept', 'application/json').expect('Contect-Type', /json/).expect(200).end((err, res) => {
                            var answerToUpdate = res.body;
                            answerToUpdate.content = "Brand new answer content";
                            request(app)
                                .put(`/api/answers/${answerToUpdate.id}`)
                                .send(answerToUpdate)
                                ...
                                .end((err, res) => {
                                    ...
                                    chaiExpect(res.body).have.property("content");
                                    chaiExpect(res.body.content).to.be.deep.equal("Brand new answer content");
                                    done();
                                });
                        });
                    });
            ...
            User.sync({ force: true }).then(() => {
                request(app).post('/api/auth/signup').send(testData.user1).set('Accept', 'application/json').expect('Contect-Type', /json/).expect(200).end((err, res) => {
                    ...
                    Answer.sync({ force: true }).then(() => {
                        request(app).post('/api/answers').send(testData.answer2).set('Authorization', `Bearer ${token}`).set('Accept', 'application/json').expect('Contect-Type', /json/).expect(200).end((err, res) => {
                            var answerToUpdate = res.body;
                            answerToUpdate.body = "Brand new question body";
                            request(app)
                                .put(`/api/answers/${answerToUpdate.id}`)
                                .send(answerToUpdate)
                                //.set('Authorization', `Bearer ${token}`)
